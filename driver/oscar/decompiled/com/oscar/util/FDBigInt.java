...
class FDBigInt {
    ...
    FDBigInt(FDBigInt paramFDBigInt) {
        this.nWords = paramFDBigInt.nWords;
    ...
    void lshiftMe(int paramInt) throws IllegalArgumentException {
        if (paramInt <= 0) {
            if (paramInt == 0) {
                return;
            }
    ...
    FDBigInt mult(int paramInt) {
        long l1 = paramInt;
        int[] arrayOfInt = new int[l1 * (long)(this.data[this.nWords - 1] & 0xFFFFFFFF) > 0xFFFFFFFL ? this.nWords + 1 : this.nWords];
        long l2 = 0L;
        for (int i = 0; i < this.nWords; ++i) {
            arrayOfInt[i] = (int)(l2 += l1 * (long)(this.data[i] & 0xFFFFFFFF));
            l2 >>>= 32;
        }
        if (l2 == 0L) {
    ...
    FDBigInt mult(FDBigInt paramFDBigInt) {
        ...
        for (i = 0; i < this.nWords; ++i) {
            ...
            for (int j = 0; j < paramFDBigInt.nWords; ++j) {
                arrayOfInt[i + j] = (int)(l2 += (long)(arrayOfInt[i + j] & 0xFFFFFFFF) + l1 * (long)(paramFDBigInt.data[j] & 0xFFFFFFFF));
                l2 >>>= 32;
            }
            arrayOfInt[i + j] = (int)l2;
        }
        for (i = arrayOfInt.length - 1; i > 0 && arrayOfInt[i] == 0; --i) {
        }
    ...
    FDBigInt add(FDBigInt paramFDBigInt) {
        ...
        for (i = 0; i < j; ++i) {
            l += (long)(arrayOfInt1[i] & 0xFFFFFFFF);
            if (i < k) {
                l += (long)(arrayOfInt2[i] & 0xFFFFFFFF);
            }
            arrayOfInt3[i] = (int)l;
            l >>= 32;
        }
    ...
    int cmp(FDBigInt paramFDBigInt) {
        ...
        if (this.nWords < paramFDBigInt.nWords) {
            j = this.nWords - 1;
            for (i2 = paramFDBigInt.nWords - 1; i2 > j; --i2) {
                if (paramFDBigInt.data[i2] == 0) continue;
                return -1;
            }
        }
        for (i = this.nWords - 1; i > 0 && this.data[i] == paramFDBigInt.data[i]; --i) {
        }
        int j2 = this.data[i];
    ...
    int quoRemIteration(FDBigInt paramFDBigInt) throws IllegalArgumentException {
        int k;
        if (this.nWords != paramFDBigInt.nWords) {
        ...
        long l2 = 0L;
        for (int j = 0; j <= i; ++j) {
            this.data[j] = (int)(l2 += (long)(this.data[j] & 0xFFFFFFFF) - l1 * (long)(paramFDBigInt.data[j] & 0xFFFFFFFF));
            l2 >>= 32;
        }
        if (l2 != 0L) {
            long l3 = 0L;
            while (l3 == 0L) {
                l3 = 0L;
                for (k = 0; k <= i; ++k) {
                    this.data[k] = (int)(l3 += (long)((this.data[k] & 0xFFFFFFFF) + (paramFDBigInt.data[k] & 0xFFFFFFFF)));
                    l3 >>= 32;
                }
                if (l3 != 0L && l3 != 1L) {
        ...
        long l3 = 0L;
        for (k = 0; k <= i; ++k) {
            this.data[k] = (int)(l3 += 10L * (long)(this.data[k] & 0xFFFFFFFF));
            l3 >>= 32;
        }
        if (l3 != 0L) {
